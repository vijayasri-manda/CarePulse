            ...
            <table className="custom-table">
              ...
              <tbody>
                {paginatedBills.map((b) => (
                  <tr key={b.id}>
                    <td>#INV-{b.id}</td>
                    <td>{b.patient ? formatPatientName(b.patient.name) : 'Unknown'}</td>
                    <td>${(b.consultationFee || 0).toFixed(2)}</td>
                    <td>${(b.medicineCharges || 0).toFixed(2)}</td>
                    <td>${(b.labCharges || 0).toFixed(2)}</td>
                    <td style={{ fontWeight: '700', color: 'var(--text-primary)' }}>${(b.totalAmount || 0).toFixed(2)}</td>
      ...
      >
        <form onSubmit={handleSubmit} style={styles.form}>
          <div className="form-group">
            ...
            >
              {patients.map(p => (
                <option key={p.id} value={p.id}>{formatPatientName(p.name)} (ID: #{p.id})</option>
              ))}
            </select>
          </div>
